...
class _FakeMessage:
    def __init__(self, msg_id: int = 1, text: str = "",
                 date: Any = None) -> None:
        self.id = msg_id
        self.text = text
        self.raw_text = text
...
class _FakeEvent:
    def __init__(self, chat_id: int = -100111,
                 message: Optional[_FakeMessage] = None,
                 sender: Optional[_FakeSender] = None) -> None:
...
class TestOnNewMessage:
    ...
    def test_target_chat_valid_text_writes_file(
        ...
        msg = _FakeMessage(
            msg_id=42, text="BTC perp breakout signal",
    ...
    def test_non_target_chat_ignored(
        ...
        msg = _FakeMessage(msg_id=99, text="some text")
    ...
    def test_empty_text_ignored(
        ...
        msg = _FakeMessage(msg_id=77, text="")
        event = _FakeEvent(chat_id=-100111, message=msg)
        self._run(listener._on_new_message(event))
        assert listener.messages_processed == 0
    ...
    def test_writer_exception_does_not_kill_listener(
        ...
        msg = _FakeMessage(
            msg_id=88, text="BTC perp signal",
            date=datetime(2026, 5, 20, tzinfo=timezone.utc),
        )
        event = _FakeEvent(chat_id=-100111, message=msg)
        with patch(
    ...
    def test_sender_lookup_failure_still_writes(
        ...
        msg = _FakeMessage(
            msg_id=55, text="Polymarket odds shift",
            date=datetime(2026, 5, 20, tzinfo=timezone.utc),
        )

        class _EventBadSender:
            chat_id = -100333
            message = msg
            async def get_sender(self):
                raise RuntimeError("sender lookup failed")

        self._run(listener._on_new_message(_EventBadSender()))
        assert listener.messages_processed == 1
        captures_dir = inbox_dir / "Telegram-Captures"
        files = list(captures_dir.iterdir())
        assert len(files) == 1
